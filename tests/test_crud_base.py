...
def _check_menu(menu: Menu):
    assert isinstance(menu, Menu)
    for field_name in ('id', 'title', 'description', 'submenus'):
        assert hasattr(menu, field_name), field_name
...
async def test_save(get_crud_base, get_test_session):
    menu = await get_crud_base._save(get_test_session, Menu(**d.MENU_POST_PAYLOAD))
    assert menu
    _check_menu(menu)
...
async def test_save_exception(get_crud_base, get_test_session):
    with pytest.raises(HTTPException) as exc_info:
        [await get_crud_base._save(get_test_session, М(**POST_SAVE_DATA)) for _ in range(2)]
